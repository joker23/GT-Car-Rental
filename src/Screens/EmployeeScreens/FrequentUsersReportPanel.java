...
public class FrequentUsersReportPanel extends JPanel{
	//GUI components
	...
	private String[][] frequentUsersInfo;
	...
	public FrequentUsersReportPanel(JFrame frame, JPanel panel){
		QueryAdaptor.connect();
		try {
			this.frequentUsersInfo = QueryAdaptor.getFrequentUsersReport();
		} catch (Exception e) {
		...
		this.setBorder (BorderFactory.createTitledBorder (BorderFactory.createEtchedBorder (), "Frequent Users Report", TitledBorder.CENTER, TitledBorder.TOP));
		...
		model.addColumn("Driving Plane");
		model.addColumn("No of Reservations per Month");
	...
	private void addRowsToTable(){
		for(int i = 0; i<frequentUsersInfo.length; i++){
			//adding each subarray of strings to the table
			model.addRow(frequentUsersInfo[i]);
		}
	}
